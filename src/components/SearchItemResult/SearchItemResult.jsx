...
export default function SearchItemResult({ products, filters }) {
  const { pagination, products: productList } = products
  const navigate = useNavigate()

  const searchSortBy = (sortBy, order) => {
    ...
    navigate(path.home + `?${qs.stringify({ ..._filters, sortBy })}`)
  }

  const handleActiveOptionSort = value => classname({ active: value === filters.sortBy })
  ...
  const arrowTop = type => {
    ...
    const page = type === 'prev' ? Number(_filters.page) - 1 : Number(_filters.page) + 1
    navigate(path.home + `?${qs.stringify({ ..._filters, page })}`)
  }
  ...
  return (
    <div>
      <S.SortBar>
        ...
        <S.SortByOptions>
          <S.SortByOptionOption className={handleActiveOptionSort('view')} onClick={() => searchSortBy('view')}>
            Phổ biến
          </S.SortByOptionOption>
          <S.SortByOptionOption
            className={handleActiveOptionSort('createdAt')}
            onClick={() => searchSortBy('createdAt')}
          >
            Mới nhất
          </S.SortByOptionOption>
          <S.SortByOptionOption className={handleActiveOptionSort('sold')} onClick={() => searchSortBy('sold')}>
            Bán chạy
          </S.SortByOptionOption>
          <S.SortByPrice
            onChange={event => searchSortBy(...event.target.value.split(':'))}
        ...
        <S.MiniPageController>
          ...
          <S.ButtonControllerPrev
            onClick={() => arrowTop('prev')}
            disabled={Number(pagination?.page) <= 1 ? true : false}
          >
          ...
          <S.ButtonControllerNext
            onClick={() => arrowTop('next')}
            disabled={pagination?.page === pagination?.page_size ? true : false}
          >
      ...
      <S.ProductList>
        {productList && productList.map((item, index) => <ProductItem key={index} product={item} />)}
      </S.ProductList>
